...
import static com.bn.Particle.ParticleDataConstant.BLEND_FUNC;
import static com.bn.Particle.ParticleDataConstant.DST_BLEND;
import static com.bn.Particle.ParticleDataConstant.END_COLOR;
import static com.bn.Particle.ParticleDataConstant.GROUP_COUNT;
import static com.bn.Particle.ParticleDataConstant.LIFE_SPAN_STEP;
import static com.bn.Particle.ParticleDataConstant.MAX_LIFE_SPAN;
import static com.bn.Particle.ParticleDataConstant.RADIS;
import static com.bn.Particle.ParticleDataConstant.SRC_BLEND;
import static com.bn.Particle.ParticleDataConstant.START_COLOR;
import static com.bn.Particle.ParticleDataConstant.THREAD_SLEEP;
import static com.bn.Particle.ParticleDataConstant.VY;
import static com.bn.Particle.ParticleDataConstant.X_RANGE;
import static com.bn.Particle.ParticleDataConstant.Y_RANGE;
import static com.bn.Particle.ParticleDataConstant.Z_RANGE;
import static com.bn.Particle.ParticleDataConstant.lock;
...
public class ParticleSystem implements Comparable<ParticleSystem>
{
    ...
    int CURR_INDEX;
    public ParticleSystem(float positionx, float positiony, float positionz, ParticleForDraw fpfd, int count,int index)
    {
...
//                    points[i*4]=px;//将粒子位置的x坐标存入points数组中
//                    points[i*4+1]=py;//将粒子位置的y坐标存入points数组中
//                    points[i*4+2]=vz;//将粒子x方向的速度存入points数组中
                    points[i*4+3]=10.0f;//将粒子的当前生命期存入points数组中----为10时，粒子处于没有被激活状态，不为10时，粒子处于活跃状态
                }else//当前生命期小于最大生命期时----计算粒子的下一位置坐标
                {
                    points[i*4]+=points[i*4+1];//计算粒子位置的x坐标
