...
public class LoadedObjectVertexNormalTex {
	...
	int vCount=0;
	public LoadedObjectVertexNormalTex(MyGLSurFaceView mv, float[] vertices, float[] normals, float texCoors[])
	{
		//初始化顶点坐标与着色数据
	...
	{
		//加载顶点着色器的脚本内容
		mVertexShader=ShaderUtil.loadFromAssetsFile("vertex_sence.sh", mv.getResources());
		//加载片元着色器的脚本内容
		mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_sence.sh", mv.getResources());
		//基于顶点着色器与片元着色器创建程序
	...
	public void drawSelf(int texId,float color[])
	{
		...
		//将颜色控制传入着色器程序
		GLES30.glUniform4f(maColorHandle, color[0], color[1], color[2], color[3]);
		// 将顶点位置数据传入渲染管线
